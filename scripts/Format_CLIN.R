...
selected_cols <- c("patient", "Arm", "pCR.status")
...
colnames(clin) <- c("patient", "drug_type", "response.other.info", "sex", "primary", "rna", "rna_info", "age", "stage", "recist", "t.os", "t.pfs", "histo", "os", "pfs", "dna", "dna_info", "response")

# Correct "response.other.info" format.
clin$response.other.info <- ifelse(clin_original$pCR == 0, "NR",
                                   ifelse(clin_original$pCR == 1, "R", NA))
...
clin <- annotate_tissue(clin=clin, study='Wolf2', annotation_tissue= annotation_tissue, check_histo=FALSE)
...
clin <- add_column(clin, treatmentid=annotate_drug('Wolf2', clin$drug_type, annotation_drug), .after='tissueid')

# Update 'drug_type' column based  category for specific 'treatmentid'
clin$drug_type[clin$treatmentid == 'Paclitaxel' ] <- 'chemo'
clin$drug_type[clin$treatmentid == 'Durvalumab/Olaparib'] <- 'IO+targeted'
